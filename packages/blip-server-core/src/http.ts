...
import type { StorageConfig, VideoPrivacy, VideoRecord } from "./domain/model.ts"
import { Auth } from "./domain/auth.ts"
import { ObjectStorage } from "./domain/object-storage.ts"
import { Repository } from "./domain/repository.ts"
import { Vault } from "./domain/vault.ts"
...
export const layer = (setupToken: string, publicOrigin: string) =>
  HttpApiBuilder.layer(BlipApi, { openapiPath: "/openapi.json" }).pipe(
